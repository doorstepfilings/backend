"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RMController = void 0;
const common_1 = require("@nestjs/common");
const api_response_1 = require("../../../../shared/http/api-response");
const jwt_auth_guard_1 = require("../../../identity/infrastructure/auth/jwt-auth.guard");
const roles_decorator_1 = require("../../../identity/infrastructure/auth/roles.decorator");
const roles_guard_1 = require("../../../identity/infrastructure/auth/roles.guard");
const current_auth_user_decorator_1 = require("../../../identity/presentation/http/current-auth-user.decorator");
const user_services_service_1 = require("../../application/user-services.service");
let RMController = class RMController {
    userServicesService;
    constructor(userServicesService) {
        this.userServicesService = userServicesService;
    }
    async getServiceRequests(authUser) {
        const services = await this.userServicesService.getRmServices(authUser.userId);
        return (0, api_response_1.successResponse)(services);
    }
    async assignAccountant(data) {
        const result = await this.userServicesService.assignAccountantToService(data.userServiceId, data.accountantId);
        return (0, api_response_1.successResponse)(result);
    }
};
exports.RMController = RMController;
__decorate([
    (0, common_1.Get)('service-requests'),
    __param(0, (0, current_auth_user_decorator_1.CurrentAuthUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], RMController.prototype, "getServiceRequests", null);
__decorate([
    (0, common_1.Post)('assign-accountant'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], RMController.prototype, "assignAccountant", null);
exports.RMController = RMController = __decorate([
    (0, common_1.Controller)('rm'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, roles_guard_1.RolesGuard),
    (0, roles_decorator_1.Roles)('rm'),
    __metadata("design:paramtypes", [user_services_service_1.UserServicesService])
], RMController);
//# sourceMappingURL=rm.controller.js.map