...
function readRequiredString(env: EnvLike, key: string, allowEmpty = false) {
  const value = env[key];
...
function readOptionalNumber(env: EnvLike, key: string, fallback: number) {
  const value = env[key];
...
export function getPrimaryDatabaseConfig(env: EnvLike = process.env) {
  return getDatabaseConfig(env, 'DB', true)!;
}

export function getLegacyDatabaseConfig(env: EnvLike = process.env) {
...
export function getPrismaDatabaseUrl(env: EnvLike = process.env) {
