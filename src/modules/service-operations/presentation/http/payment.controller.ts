...
export class PaymentController {
  ...
  async createOrder(
    ...
    @Body('user_service_id', ParseIntPipe) userServiceId: number,
  ) {
  ...
  async failPayment(
    @CurrentAuthUser() user: { userId: number },
    @Body('payment_id', ParseIntPipe) paymentId: number,
  ...
  async downloadInvoice(
    ...
    const pdfBuffer = await this.paymentService.downloadInvoice(
      user.userId,
      id,
    );
